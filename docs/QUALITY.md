...
| `CAP-006` | Snapshot-relative authority resolution maps retained exact-match evidence to distinct zero, one, or multiple outcomes and permits a positive refinement only for exactly one matching record entry | Private-field Go types, three manually authored bounded TSV tables, exact revision and target-local multiplicity cases, zero and forged-value tests, maximum-bound properties, and fuzzing across record order | Define resolution-source and closure semantics, then ratify Declaration Authority Resolution, Attribution Scope, and Catalog Lookup Closure before product registration absence or wire projection |
