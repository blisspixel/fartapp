...
func TestWalkCLIAllOperationsAndRetainedReconstruction(t *testing.T) {
	...
	if code := runWithInput([]string{"fartapp", "walk", "witness", "-", "--format", "json"}, bytes.NewReader(input), &stdout, &stderr); code != 0 || stderr.Len() != 0 {
	...
	for _, operation := range []string{"predict", "simulate", "inspect", "explain", "branch", "certify", "witness", "reconstruct"} {
		for _, format := range []string{"text", "json"} {
			t.Run(operation+"/"+format, func(t *testing.T) {
				stdout.Reset()
				stderr.Reset()
				code := runWithInput([]string{"fartapp", "walk", operation, "-", "--format", format}, bytes.NewReader(retained), &stdout, &stderr)
	...
	stdout.Reset()
	stderr.Reset()
	code := runWithInput([]string{"fartapp", "walk", "reconstruct", "-", "--format", "json"}, bytes.NewReader(mismatch), &stdout, &stderr)
	var report walkcase.Report
...
func TestWalkCLIInputOutputAndArgumentBoundaries(t *testing.T) {
	for _, args := range [][]string{
		...
		var stdout, stderr bytes.Buffer
		if code := runWithInput(args, strings.NewReader(""), &stdout, &stderr); code != 1 || stdout.Len() != 0 || stderr.Len() == 0 {
	...
	for _, input := range []io.Reader{nil, walkCLIErrorReader{}, strings.NewReader(strings.Repeat("x", walkcase.MaxInputBytes+1)), strings.NewReader("{}")} {
		var stdout, stderr bytes.Buffer
		code := runWithInput([]string{"fartapp", "walk", "simulate", "-", "--format", "json"}, input, &stdout, &stderr)
		var report walkcase.Report
	...
	for _, source := range []string{"testdata/walk/isothermal-choked.json", "-"} {
		for _, format := range []string{"text", "json"} {
			var stderr bytes.Buffer
			code := runWithInput([]string{"fartapp", "walk", "simulate", source, "--format", format}, strings.NewReader("{}"), failingWriter{}, &stderr)
	...
	var stderr bytes.Buffer
	if code := runWithInput([]string{"fartapp", "walk", "--help"}, nil, failingWriter{}, &stderr); code != 1 {
...
func TestWalkCLIFileAndStdinReportsAreIdentical(t *testing.T) {
	...
	for _, operation := range []string{"simulate", "branch", "witness"} {
		...
		if code := runWithInput([]string{"fartapp", "walk", operation, path, "--format", "json"}, nil, &file, &stderr); code != 0 {
			t.Fatalf("file status %d: %s", code, stderr.String())
		}
		if code := runWithInput([]string{"fartapp", "walk", operation, "-", "--format", "json"}, bytes.NewReader(input), &stdin, &stderr); code != 0 {
