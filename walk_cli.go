...
There is no archive, case commitment, certificate authority, or physical audio.
...
func runWalk(args []string, stdin io.Reader, stdout, stderr io.Writer) int {
	...
	if report.Predicted() {
		return writeValue(stdout, stderr, options.format, report, formatWalk)
	}
	if options.format == outputJSON {
		if writeValue(stdout, stderr, options.format, report, formatWalk) != 0 {
			return 1
		}
		return 1
	}
...
func readWalk(source, operation string, stdin io.Reader) walkcase.Report {
	...
	return walkcase.Run(data, operation)
}
