...
const expectedAtemporalScenarioJSONSHA256 = "cfbad5fc2f59727285214a2ba865192bdf86004496c147b683aff06d3a718a99"
const expectedEarthScenarioJSONSHA256 = "77cecf0ed6413bb83ebd291f6729012f944d6261f5d8fe8fe028ca2113bc634c"
const expectedMinimalOpaqueScenarioJSONSHA256 = "e41d89d827c90bc03260154726d66fcd6e1b8bb0ad992d3295c62169436f096b"
...
func TestScenarioCLITextAndJSONFixtures(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if code := Run(test.args, bytes.NewReader(input), &stdout, &stderr); code != 0 {
	...
	invokeJSON := func(args []string, stdin []byte) []byte {
		...
		var stdout bytes.Buffer
		var stderr bytes.Buffer
		if code := Run(args, bytes.NewReader(stdin), &stdout, &stderr); code != 0 {
			t.Fatalf("exit code = %d, stderr = %q", code, stderr.String())
		}
...
func TestScenarioCLIInputFailures(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			var stdout bytes.Buffer
			var stderr bytes.Buffer
			code := Run(append(test.args, "--format", "json"), test.stdin, &stdout, &stderr)
			if code != 1 || stderr.Len() != 0 {
